...
public class Day07 {
    ...
    private static List<Bag> createInnerBags(List<String> bagsText) {
        ...
        for (String bagText : bagsText) {
            ...
            List<Integer> amounts = getAmounts(bagText);
            List<String> innerBagColors = getInnerBagColors(bagText);

            for (int i = 0; i < amounts.size(); i++) {
                bag.getInnerBags().put(new Bag(innerBagColors.get(i)), amounts.get(i));
            }
            bags.add(bag);
        }
        return bags;
    ...
    private static List<String> getInnerBagColors(String bagRuleText) {
        ...
        String[] allAmountBag = bagRuleText.split(", ");

        for (String amountBag : allAmountBag) {
            amountBag = amountBag.replace("bags", "");
    ...
    private static List<Integer> getAmounts(String bagRuleText) {
        ...
        for (int i = 0; i < bagRuleText.length(); i++) {
            ...
            if (bagRuleText.charAt(i) >= 49 && bagRuleText.charAt(i) <= 57) {
                amounts.add(Integer.parseInt(String.valueOf(bagRuleText.charAt(i))));
            }
        }
        return amounts;
    }
